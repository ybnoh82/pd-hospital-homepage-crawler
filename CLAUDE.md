...
# 한 병원 크롤링 실행 (러너). 기본 Opus 4.8 · effort medium · 예산캡 $3 · 시간캡 900s(15분)
...
# 가장 어려운 사이트: --effort high (기본이 이미 Opus). 비용 우선: --model claude-sonnet-4-6 --effort low
...
2. **모델·effort·이중 캡** — `model`(기본 Opus 4.8)·`effort`(기본 medium, Sonnet/Opus만 — Haiku는 미지원이라 모델 가드)·`max_budget_usd`(기본 $3, 느슨한 백스톱)를 건다. **시간이 단일 바인딩 제약**이다: `DEFAULT_TIME_LIMIT_S`(900s, 15분) wall-clock 하드캡 + 78%(`SOFT_DEADLINE_RATIO`) 소프트 데드라인. `max_buffer_size`도 50MB로 키운다(기본 1MB는 base64 스크린샷 한 장에 리더가 죽는다 — vision 핵심).
...
**[2026-06-17 기본 정책: Opus 4.8 · effort medium] sample10 6조합(sonnet/opus × low/med/high)×10병원 매트릭스 실측으로 기본을 Opus 4.8·effort medium으로 정했다(예산캡 $3·시간캡 900s 유지).** Opus가 매칭률(제품 52~56% vs Sonnet 34~51%)·운영정보 캡처(전화·진료시간 100% vs Sonnet 60~90%)에서 우위였고 $3 예산캡에 먼저 닿아 빨랐다(중앙 400~520s). 반면 Sonnet/high(직전 기본)는 900s 시간캡에 걸려 느리고(중앙 860s) 비용 기록까지 잃어 dominated였다. effort는 Opus에서 매칭 품질이 평탄해 medium이 시술 수집량·비용 균형점. 비용 최우선이면 `--model claude-sonnet-4-6 --effort low`. (아래 옛 수치는 저비용 캡 측정 기록이다.)

강남 5개 실사이트(SPA·성형외과·미용의원·프랜차이즈·피부과)로 측정한 결론:

- **(구 저비용 캡 기준) 비용·시간은 캡으로 구조적으로 보장된다.** 시간 하드캡 270s(4.5분) + 비용 백스톱으로 모든 실행이 ≤4.5분·≤~$1(실측 자연완주/캡 $0.77–$1.03). 단 완전성을 희생했다(매칭·부차 정보 누락).
- **모델이 가장 큰 레버.** Haiku 4.5는 이 멀티스텝 에이전트(SPA·매칭·vision)에서 조기 종료·미저장으로 **완주 실패**(측정). Opus 4.8이 기본(매트릭스 실측 품질·속도 우위), Sonnet 4.6은 비용 우선 시(`--model`).
