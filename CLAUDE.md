...
# 한 병원 크롤링 실행 (러너). 기본 Sonnet 4.6 · effort high · 예산캡 $3 · 시간캡 900s(15분)
...
2. **모델·effort·이중 캡** — `model`(기본 Sonnet 4.6)·`effort`(기본 high, Sonnet/Opus만 — Haiku는 미지원이라 모델 가드)·`max_budget_usd`(기본 $3, 느슨한 백스톱)를 건다. **시간이 단일 바인딩 제약**이다: `DEFAULT_TIME_LIMIT_S`(900s, 15분) wall-clock 하드캡 + 78%(`SOFT_DEADLINE_RATIO`) 소프트 데드라인. `max_buffer_size`도 50MB로 키운다(기본 1MB는 base64 스크린샷 한 장에 리더가 죽는다 — vision 핵심).
...
**[2026-06-17 기본 정책 변경] 기본값을 완전성 우선으로 올렸다: effort high · 예산캡 $3 · 시간캡 900s(15분).** 저비용 캡(low·$0.90·270s)은 미용·성형 사이트에서 카탈로그 매칭·스키마 정합 최종저장을 못 끝내고 끊겨 빈 결과가 잦았다(실측: sample10 재크롤 5/5 EMPTY — 데이터 손실 버그는 `output_scheme.py`에서 수정 `d80011e`, 그래도 조기종료/지연저장 꼬리는 남음). 비용·속도가 더 중요하면 `--effort low --budget 0.9 --time-limit 270`으로 내릴 수 있다. 아래 수치는 그 저비용 캡으로 측정한 기록이다.
