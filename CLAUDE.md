...
> **`EVOLUTION.md`** — "스스로 진화하는 크롤러" 전체 설계도(골든 회귀 하니스 → 배치 드라이버 → 독립 검증 sub-agent → 진화 엔진, Phase 0~6). **앞으로의 방향·다음 작업의 진실원** — 다음 작업은 여기 Phase 0부터. (운영 규칙·측정된 사실은 이 문서, 크롤 행동규칙은 `SKILL.md`.)
...
0. **사전 트리아지(`triage.py`)** — 크롤 전에 curl로 싸게 분류한다(LLM·브라우저 없이 ~$0·수초). 죽은/접속불가 도메인이나 명백한 비타깃 진료과(안과·이비인후과 등, 강한 미용 신호 0)면 비싼 에이전트를 띄우지 않고 유효 스켈레톤만 쓰고 **exit 2**로 제외한다(배치는 재시도 안 함). 그 외엔 크롤로 진행하며 사이트 형태(text/image/spa) 힌트를 프롬프트에 주입한다. **스킵은 보수적**이다 — 잘못 스킵하면 진짜 타깃을 잃으므로(최악의 에러), 강한 미용 신호가 조금이라도 있거나 SPA로 내용 판단이 안 되면 무조건 크롤한다. `--no-triage`로 끈다. 규칙 상세는 `triage.py` docstring.
...
6. **유효성·무손실 보장(repair)·비용 백필** — 끝나면 `output_scheme.py --repair`로 결과를 **항상 스키마 통과 형태로 고친다**(잘 쓴 파일은 no-op, 무효 부분만 제거, 미저장이면 스켈레톤 생성). 같은 단계에서 **`aggregate_from_treatments`가 시술(`treatments[].product_name/equipment_name`)에 거둔 제품·장비 이름을 최상위 `unmatched`로 자동 집계**해 *거둔 건 절대 잃지 않게* 한다(repair가 무효 항목을 삭제하다 1순위 가치 출력을 흘리던 구멍을 막음). 그 뒤 `ResultMessage`의 실측 비용·소요시간을 `crawl_metadata.cost` 빈 칸에 채운다. 유효 파일은 항상 남고, **유의미한 데이터가 있으면 exit 0(USEFUL), 빈약하면 exit 1(EMPTY → 배치 재시도), 트리아지 제외는 exit 2(재시도 안 함)**.
...
- **머신/병렬:** 24GB·12코어 기준 한 실행 ~1GB(claude CLI+playwright+chromium+python). **동시 8–12개 안전**, CPU 아닌 **메모리 바인딩**. 단 8,000 배치의 진짜 천장은 머신이 아니라 **API 레이트리밋(TPM/RPM)** — 배치 드라이버는 429 백오프 필수(EVOLUTION §4).
- **데이터 무손실(결정론적 보장):** repair가 무효 항목을 삭제하다 시술에 잘 거둔 제품·장비가 조용히 증발하던 버그가 있었다(톡스앤필: 시술61에 제품17·장비13이 박혔는데 products/equipments는 0, USEFUL로 통과). `aggregate_from_treatments`로 해결 — 시술의 이름을 unmatched로 자동 집계해 **거둔 건 절대 안 흘린다**.
- **매칭 품질은 이미 양호 — 결정론적 매칭 패스 금지.** opus/high로 sample10 10개 실측 결과, unmatched의 낮은 매칭률은 누락이 아니라 정당한 보류(일반명·다른 제품)다. 카탈로그에 대고 기계적으로 매칭하면 거짓매칭을 양산한다(`국산보톡스→보톡스`, `피코프락셀→프락셀`). 매칭은 에이전트(또는 EVOLUTION의 검증 sub-agent) 판단에 맡긴다.
- **트리아지 효과(curl·$0):** 비타깃 진료과(안과·이비인후과 등)·죽은/주차 도메인을 크롤 전에 차단. 실측: 실제 안과·이비인후과·가정의학·dead 7개 전부 SKIP(각 ~$2–3 절약), sample10 타깃 10개 전부 CRAWL. 강남 CSV 1,405개 중 ~90개(6%)가 명백한 비타깃.
- **알려진 갭(미해결):** 시간캡으로 끊긴 실행은 `ResultMessage`가 안 와 비용 백필이 안 된다(`cost=None`). 배치 비용 집계엔 경과시간 기반 추정 등 보완이 필요(EVOLUTION §4 배치 드라이버에서 다룸).
